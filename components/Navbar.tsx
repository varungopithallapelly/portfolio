    ...
    <>
      ...
      >
        ...
        <button onClick={toggleTheme} title="Toggle theme" style={{
          ...
          color: 'var(--accent)', fontSize: '15px', transition: 'all 0.2s',
          flexShrink: 0,
        }}>
          {isDark ? '☀' : '☾'}
        </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px', alignItems: 'center' }}>
          {socials.map(s => (
            <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" title={s.label} style={{
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--accent)'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)' }}
